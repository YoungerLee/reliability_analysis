...
class DeviceListWidget(QDialog):
    ...
    def initFirstPage(self):
        ...
        # 记录查询
        limitIndex = 0
        self.model.getDeviceByPages(limitIndex, self.__pageRecordCount)

    def getTotalRecordCount(self):
        return self.model.getTotalRecordCount()
...
class RecordListDialog(QDialog):
    ...
    def getTotalRecordCount(self):
        return self.model.getTotalRecordCount(self.dev_id)
    ...
    def on_addDataButton_clicked(self):
        ...
        if dialog.exec_():
            ...
            record = Record(run_time=run_time, env_temp=env_temp, kni_temp=kni_temp, rpa=rpa, axis=axis)
            dev_id = self.dev_id
            if dev_id > 0:
